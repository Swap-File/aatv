    ...
    if (raindrops[i].enabled == FALSE) {
      if ((((float) rand ()) / (float) RAND_MAX) < aatv->rain_spawn_rate) {
        ...
        if (obstructed == FALSE) {
          ...
          raindrops[i].length =
              gst_aatv_rand_range (aatv->rain_length_min,
              aatv->rain_length_max);
          raindrops[i].delay =
              gst_aatv_rand_range (aatv->rain_delay_min, aatv->rain_delay_max);
...
gst_aatv_class_init (GstAATvClass * klass)
{
  ...
  g_object_class_install_property (G_OBJECT_CLASS (klass), PROP_RAIN_DELAY_MAX,
      g_param_spec_int ("rain-delay-max", "rain-delay-max",
          "Maximum frame delay between rainp motion", 0, G_MAXINT, 0,
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
